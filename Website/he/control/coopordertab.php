...
<?php 
  if ($oTabInfo->ID > 0)
  {
    ...
    if ($oTabInfo->CheckCoopOrderSumsPermission())
      echo '<td class="headmainlabel">סכום לקואופ:&nbsp;</td><td class="headmaindata">' , $oTabInfo->CoopTotal , '</td>';
    
    echo '<td>&nbsp;&nbsp;</td>';
    if ($oTabInfo->Capacity != NULL)
      echo '<td class="headmaindata">' , $oTabInfo->Capacity , '&nbsp;תפוסה</td>';
    echo '<td>&nbsp;&nbsp;</td>';
  ...
  <tr>
    <td>
          <?php             
            ...
            echo '<li id="tabCoopOrderItem" onclick="javascript:ToggleTabDisplay(\'tabCoopOrder\');" ', $sCoopOrderTabSelected , ' title="הזמנת הקואופרטיב">', 
                $oTabInfo->CoopOrderTitle, '</li>';
            if ($bHasPickupLocation) {
              echo '<li id="tabPickupLocationItem" onclick="javascript:ToggleTabDisplay(\'tabPickupLocation\');"  ', $sPickupLocationTabSelected , ' title="מקום האיסוף">' ,$oPLTabInfo->MainTabName, '</li>';
            }
...
<?php
 ...
 WriteTabElement('כותר ההזמנה','&gt;&gt;', $g_sRootRelativePath . 'coord/cooporder.php?id=' . $oTabInfo->ID, 
        $oTabInfo->Page == CoopOrderTabInfo::PAGE_ENTRY );

if ($oTabInfo->ID > 0)
{
  WriteTabElement('מקומות איסוף','&gt;&gt;', 
  ...
  if ($oTabInfo->HasPermission(CoopOrderTabInfo::PROPERTY_PERMISSION_COOP_ORDER_COORD))
  {
    if ($oTabInfo->CheckCoopOrderCopyPermission())
      WriteTabElement('העתקה','', $g_sRootRelativePath . 'coord/coopordercopy.php?id=' . $oTabInfo->ID , FALSE);
  } 
  ...
  if ($oTabInfo->CheckCoopOrderSetCoordPermission())
  {
    ...
    WriteTabElement('תיאום','', $sUrl , FALSE);
  }
}
