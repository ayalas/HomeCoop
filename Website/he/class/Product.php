...
class Product extends SQLBase {
  ...
  {   
    ...
    {
      $sMessage = sprintf('העלאת הקובץ %s נכשלה.', $oUploader->OriginalFileName);
      //analize errors
      switch($nResponse)
      {
        case FileUploader::RESPONSE_FILE_OBJECT_NOT_SET:
          $sMessage .= ' נראה שהסיבה לכך היא באג בתוכנה.';
          break;
        case FileUploader::RESPONSE_MAX_FILE_SIZE_EXCEEDED:
          $sMessage .= sprintf(' גודל הקובץ חורג מהמכסה המקסימלית של %dKb', (PRODUCT_IMAGE_MAX_FILE_SIZE/1024)); //1024 bytes = 1 KB
          break;
        case FileUploader::RESPONSE_FILE_TYPE_UNSUPPORTED:
          $sMessage .= ' סוג או גודל הקובץ לא נתמך.';
          break;
        case FileUploader::RESPONSE_CANT_WRITE:
          $sMessage .= ' נראה שיש בעיית הרשאה באתר בעת ההעלאה אליו';
          break;
        case FileUploader::RESPONSE_NO_TMP_DIR:
          $sMessage .= ' נראה שיש בעיית הרשאה או הגדרות באתר בעת ההעלאה אליו - המערכת לא מוצאת תיקייה זמנית לביצוע ההעלאה אליה';
          break;
        case FileUploader::RESPONSE_UPLOADS_DIR_WRITE_FAILED:
          $sMessage .= ' הקובץ הועלה, אך העברתו מהתיקיה הזמנית לתיקיית ההעלאות נכשלה - כנראה בשל בעיית הרשאות';
          break;
        case FileUploader::RESPONSE_FILE_OBJECT_ERROR:
          $sMessage .= sprintf(' קוד/י שגיאה: %s', implode(',', $oFile['error']));
          break;
      }
  ...
  public function Validate()
  {
    ...
    if (!$this->ValidateRequiredNames(self::PROPERTY_PRODUCT_NAMES, 'שם מוצר'))
      $bValid = FALSE;
    
    if ($this->m_aData[self::PROPERTY_PRODUCER_ID] <= 0)
    {
      $g_oError->AddError(sprintf('יש לבחור %s', 'יצרן'));
      $bValid = FALSE;
    }
    
   /* if (!$this->ValidateRequiredNames(self::PROPERTY_SPEC_STRINGS, 'פירוט'))
      $bValid = FALSE;*/
    
    if ($this->m_aData[self::PROPERTY_UNIT_ID] <= 0)
    {
      $g_oError->AddError(sprintf('יש לבחור %s', 'יחידת מדידה'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_QUANTITY] === NULL)
    {
      $g_oError->AddError(sprintf('יש להזין %s', 'כמות'));
      $bValid = FALSE;
    }
    else if ($this->m_aData[self::PROPERTY_QUANTITY] <= 0)
    {
      $g_oError->AddError(sprintf('%s לא יכול להכיל ערך שלילי או אפס', 'כמות'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_PACKAGE_SIZE] != NULL && $this->m_aData[self::PROPERTY_PACKAGE_SIZE] <= 0)
    {
      $g_oError->AddError(sprintf('%s לא יכול להכיל ערך שלילי או אפס', 'גודל חבילה'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_ITEMS_IN_PACKAGE] === NULL)
    {
      $g_oError->AddError(sprintf('יש להזין %s', 'פריטים בחבילה'));
      $bValid = FALSE;
    }
    else if ($this->m_aData[self::PROPERTY_ITEMS_IN_PACKAGE] <= 0)
    {
      $g_oError->AddError(sprintf('%s לא יכול להכיל ערך שלילי או אפס', 'פריטים בחבילה'));
      $bValid = FALSE;
    }
    ...
    {
      $g_oError->AddError(sprintf('%s לא יכול להכיל ערך שלילי או אפס', 'כמות לכל פריט'));
      $bValid = FALSE;
    }     
    ...
    if ($this->m_aData[self::PROPERTY_ITEM_QUANTITY] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'כמות לכל פריט'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_ITEM_UNIT_ID] == NULL)
      {
        $g_oError->AddError( sprintf('יש להזין ערך עבור %1$s כאשר מוזן ערך עבור %2$s.', 'פריט נמדד ב',
                'כמות לכל פריט'
                ));
        $bValid = FALSE;
      }
    ...
    {
      $g_oError->AddError( sprintf('יש להזין ערך עבור %1$s כאשר מוזן ערך עבור %2$s.',
                'כמות לכל פריט', 'פריט נמדד ב'
                ));
        $bValid = FALSE;
    }
    ...
    {
      $g_oError->AddError(sprintf('%s לא יכול להכיל ערך שלילי או אפס', 'הזמנה בהפרשים של'));
      $bValid = FALSE;
    }
    ...
    {
      $g_oError->AddError(sprintf('%s לא יכול להכיל ערך שלילי', 'מקס. הזמנה'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_PRODUCER_PRICE] === NULL)
    {
      $g_oError->AddError(sprintf('יש להזין %s', 'מחיר יצרן'));
      $bValid = FALSE;
    }
    else if ($this->m_aData[self::PROPERTY_PRODUCER_PRICE] < 0)
    {
      $g_oError->AddError(sprintf('%s לא יכול להכיל ערך שלילי', 'מחיר יצרן'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_COOP_PRICE] === NULL)
    {
      $g_oError->AddError(sprintf('יש להזין %s', 'מחיר קואופרטיב'));
      $bValid = FALSE;
    }
    else if ($this->m_aData[self::PROPERTY_COOP_PRICE] < 0)
    {
      $g_oError->AddError(sprintf('%s לא יכול להכיל ערך שלילי', 'מחיר קואופרטיב'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_BURDEN] === NULL)
    {
      $g_oError->AddError(sprintf('יש להזין %s', 'מעמסה'));
      $bValid = FALSE;
    }
    ...
    {
      $g_oError->AddError('כאשר מוגדר צירוף למוצר יחידת המדידה של המוצר חייבת להיות ביחידות');
      $bValid = FALSE;
    }
  ...
  public static function AllowsPartialOrders($Unit, $Quantity, $UnitInterval, $PackageSize)
  {
    //partial orders are allowed when the quantity (weight or volume) is different than the unit interval
    //if there's no package size, there is nothing to be partial from
    if ($Unit == Consts::UNIT_ITEMS || $PackageSize == NULL)  
      return FALSE;
    
    if ($UnitInterval != NULL)
      return ($UnitInterval != $PackageSize && $Quantity != $UnitInterval);
