...
class Order extends SQLBase {
 ...
 public function ReductFromBalance()
 {
  ...
  $this->RunSQLWithParams($sSQL, array(
            ...
            'date' => $sNow,
            'desc' => 'תשלום בקיזוז אוטומטי מפקדון',
          ));
 ...
 protected function ReductFromBalanceOnPLChange()
 {
  ...
  public function ValidatePickupLocation($bFromItems)
  {
    ...
    {  
      if ($this->m_aData[self::PROPERTY_PICKUP_LOCATION_COOP_TOTAL] > $this->m_aData[self::PROPERTY_PICKUP_LOCATION_MAX_TOTAL])
      {
        if ($this->HasPermission(self::PERMISSION_COORD))
          $g_oError->AddError('שדה סה&quot;כ לקואופ במקום האיסוף של הזמנת הקואופרטיב חרג מהמגבלה שהוגדרה בשדה מכסת סכום לקואופ של מקום האיסוף.');
        $bValid = FALSE;
         ...
         $this->m_aData[self::PROPERTY_PICKUP_LOCATION_TOTAL_BURDEN] > $this->m_aOriginalData[self::PROPERTY_PICKUP_LOCATION_TOTAL_BURDEN]) )
    {
      ...
      {
        if ($this->HasPermission(self::PERMISSION_COORD))
          $g_oError->AddError('שדה סה&quot;כ מעמסה במקום האיסוף של הזמנת הקואופרטיב חרג מהמגבלה שהוגדרה בשדה קבולת משלוח של מקום האיסוף.');
        $bValid = FALSE;
        $bShowEnlargeError = TRUE;
      }
    }
    ...
    if ($bShowEnlargeError)
      $g_oError->AddError('לא ניתן להגדיל את ההזמנה הזו מעבר לתפוסה המלאה של מקום האיסוף של הזמנת הקואופרטיב');
    ...
    if (!$bFromItems && $this->m_aData[self::PROPERTY_ID] > 0) 
    {
      ...
      {
        $g_oError->AddError('מקום האיסוף שבחרת לא מאחסן חלק מהמוצרים המוזמנים.');
        $bValid = FALSE;
      }
      ...
      while($rec = $this->fetch())
      {
        ...
        {
          $g_oError->AddError('מקום האיסוף שבחרת לא מכיל מספיק שטח אחסון פנוי עבור חלק מהמוצרים המוזמנים.');
          $bValid = FALSE;
          break;
        }
      }  
  ...
  protected function CanModify()
  {
    ...
    if (!$this->m_bRunningTotalsValidation)
    {
      ...
      if ($this->m_aData[self::PROPERTY_COOP_ORDER_STATUS_OBJECT]->Status != ActiveCoopOrderStatus::Open)
      {
        if (!$bHasCoordPermission)
        {
          $this->AddError('לא ניתן לעדכן את הזמנה זו בשל מגבלת טווח התאריכים של הזמנת הקואופרטיב.');
          return FALSE;
        }
        else
    ...
    {    
      if ($this->IsLarger())
      {
        ...
        if ($this->m_aData[self::PROPERTY_ID] > 0)
          $this->AddError('לא ניתן להגדיל את ההזמנה הזו מעבר לתפוסה המלאה של הזמנת הקואופרטיב');

        //more detailed message for coordinators
        if ($bHasCoordPermission)
        {
          if ($oCoopOrderCapacity->SelectedType == CoopOrderCapacity::TypeBurden)
            $this->AddError('הסיבה לתפוסה המלאה היא ששדה סה&quot;כ מעמסה בהזמנת הקואופרטיב הגיע לערך של שדה קבולת משלוח של הזמנת הקואופרטיב. להגדלת התפוסה יש להגדיל את הערך בשדה קבולת המשלוח');
          else if ($oCoopOrderCapacity->SelectedType == CoopOrderCapacity::TypeTotal)
            $this->AddError('הסיבה לתפוסה המלאה היא ששדה סה&quot;כ לקואופ בהזמנת הקואופרטיב הגיע לערך של שדה מכסת סכום לקואופ של הזמנת הקואופרטיב. להגדלת התפוסה יש להגדיל את הערך בשדה מכסת סכום לקואופ');
        }
      }

      if ($this->m_aData[self::PROPERTY_ID] == 0)
      {
        $this->AddError('לא ניתן ליצור הזמנה חדשה כיוון שהזמנת הקואופרטיב בתפוסה מלאה');
    ...
    if ($this->m_aData[self::PROPERTY_MAX_ORDER] != NULL) //if not payment at pickup
    {
      ...
      if ($mSumForValidation < 0)
      {
        ...
        {
          $this->AddError(sprintf(' לא ניתן להגדיל את ההזמנה מעבר לסכום מוצרים כולל של %s',$this->m_aData[self::PROPERTY_MAX_ORDER]));
  ...
  protected function Validate()
  {
    ...
    if ($this->m_aData[self::PROPERTY_PICKUP_LOCATION_ID] <= 0)
    {
      $g_oError->AddError(sprintf('יש לבחור %s', 'מקום האיסוף'));
      $bValid = FALSE;
    }
  ...
  public function BuildPageTitle()
  {
    ...
    if ($this->m_aData[self::PROPERTY_ID] > 0)
    {
      ...
      {
        $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION] = 'ההזמנה שלי';
        $this->m_aData[self::PROPERTY_PAGE_TITLE] = $this->m_aData[self::PROPERTY_COOP_ORDER_NAME] . ' - ' . 
                $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION];

      }
      else
      {
        $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION] = sprintf('ההזמנה של %s', $this->m_aData[self::PROPERTY_MEMBER_NAME]);
        $this->m_aData[self::PROPERTY_PAGE_TITLE] = $this->m_aData[self::PROPERTY_COOP_ORDER_NAME] . ' - ' . 
                $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION];
      }
    }
    ...
    {
      $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION] = 'הזמנה חדשה';
      $this->m_aData[self::PROPERTY_PAGE_TITLE] = $this->m_aData[self::PROPERTY_COOP_ORDER_NAME] . ' - ' . 
                $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION];
    }
  }
