...
class CoopOrderProducer extends CoopOrderSubRecordBase {
  ...
  public function Delete()
  {
    ...
    {
      $this->m_nLastOperationStatus = parent::OPERATION_STATUS_VALIDATION_FAILED;
      $g_oError->AddError('כבר יש הזמנות עבור היצרן הזה בהזמנת הקואופרטיב הנוכחית. לא ניתן למחוק את היצרן במצב כזה. ניתן למחוק מוצרים ספציפיים והזמנותיהם ימחקו גם כן.');
      return FALSE;
    }
  ...
  public function Validate()
  {
    ...
    if ($this->m_aData[self::PROPERTY_MAX_PRODUCER_ORDER] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'מכסת הזמנה ליצרן'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_MAX_PRODUCER_ORDER] < 0)
      {
        $g_oError->AddError( sprintf('%s לא יכול להכיל ערך שלילי', 'מכסת הזמנה ליצרן'));
        $bValid = FALSE;
      }
    }
