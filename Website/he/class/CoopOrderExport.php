...
class CoopOrderExport extends CoopOrderSubBase {
  ...
  public function GetDataSetsList()
  {
    global $g_oMemberSession;
             ...
             FALSE))
     {
        ...
        $aReturn[( self::LIST_ITEM_TYPE_ORDER ) ] = 'כל הנתונים';
        $aReturn[( self::LIST_ITEM_TYPE_ORDER + self::LIST_ITEM_TYPE_SUMMARY ) ] = 'סיכום הזמנת הקואופרטיב';
     
        if ($bHasOrdersPermission)
          $aReturn[ ( self::LIST_ITEM_TYPE_MAILS ) ] = 'מיילים של כל המזמינים';
     }
     ...
     while($recPickupLoc)
     {
             ...
             FALSE))
       {
         ...
         $aReturn[$fIndex ] = sprintf('משלוח ל%s',htmlspecialchars($recPickupLoc["sPickupLocation"]));
         ...
         if ($this->AddPermissionBridge($fIndex, Consts::PERMISSION_AREA_COOP_ORDER_PICKUP_LOCATION_ORDERS, 
             ...
             FALSE))
          $aReturn[$fIndex ] = sprintf('מיילים של מזמינים ל%s',htmlspecialchars($recPickupLoc["sPickupLocation"]));
       }
             ...
             TRUE))
     {
       ...
       while($recProducer)
       {
         ...
         $aReturn[$fIndex ] = sprintf('הזמנה מ%s',  htmlspecialchars($recProducer["sProducer"]));
         $fIndex = self::LIST_ITEM_PRODUCER + self::LIST_ITEM_TYPE_MAILS + $nID;
         //must have permission to member orders to view member emails
         if ($bHasOrdersPermission)
          $aReturn[$fIndex ] = sprintf('מיילים של מזמינים מ%s',  htmlspecialchars ($recProducer["sProducer"]));
     ...
     if ($bHasOrdersPermission)
      $aReturn[ (self::LIST_ITEM_PRODUCTS + self::LIST_ITEM_TYPE_MAILS)  ] = 'מיילים של מזמיני מוצרים';
 ...
 {   
  ...
  {    
    ...
    $this->BuildSheetSummaryRowXML( TRUE,'סכום כולל',
  ...
  protected function BuildNewSheetXML($sSheetName, $bIncludeOrders, &$sheet, &$batch)
  {
    ...
    $prdh = $this->m_oXmlDoc->createElement('prdh', 'מוצר');
    $colh->appendChild($prdh);
    
    $quantityh = $this->m_oXmlDoc->createElement('quantityh', 'פריט');
    $colh->appendChild($quantityh);
    
    $priceh = $this->m_oXmlDoc->createElement('priceh', 'מחיר');
    $colh->appendChild($priceh);
    
    $packageh = $this->m_oXmlDoc->createElement('packageh', 'גודל חבילה');
  ...
  protected function BuildSheetSummaryRowXML($bIncludeOrders, $sLabel, $mSummary, &$batch)
  {
    ...
    {
      //add fee row
      $feelabel = $this->m_oXmlDoc->createElement('sumlabel', 'תוספת לקואופ');
  ...
  protected function BuildPickupLocationDetailSheetXML(&$sheet)
  {
    ...
    $sheetname = $this->m_oXmlDoc->createElement('name', $this->remove_filename_special_char(sprintf('%s- פירוט', 
  ...
  protected function BuildFooter(&$sheet)
  {
    ...
    $legend = $this->m_oXmlDoc->createElement('legend', '* - שולם ע&quot;י קיזוז מהפקדון');
