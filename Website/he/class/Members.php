...
class Members extends SQLBase {
 ...
 {
   return array( self::EXPORT_LIST_ITEM_ALL_MEMBERS_DATA => 'כל נתוני החברות/ים',
       self::EXPORT_LIST_ITEM_SELECTED_MEMBERS_EMAILS => 'מיילים של חברות/ים שנבחרו'
    );
 }
 ...
 {    
  ...
  protected function BuildXmlDoc()
  {
    ...
    $ch = $this->m_oXmlDoc->createElement('colheader', 'שם');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'שם כניסה');
    $colh->appendChild($ch);
    ...
    $ch = $this->m_oXmlDoc->createElement('colheader', 'אחוז חריגה');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'כתובת דוא&quot;ל');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'הצטרפות');
    $colh->appendChild($ch);
    ...
    while($recMember)
    {
      ...
      $rd = $this->m_oXmlDoc->createElement('djoin', $dJoined->format('j.n.Y') );
      $row->appendChild($rd);
