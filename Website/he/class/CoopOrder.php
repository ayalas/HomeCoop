...
class CoopOrder extends SQLBase {
  ...
  protected static $m_aStatusNames = array(
        ...
        self::STATUS_ACTIVE => 'פעילה',
        self::STATUS_LOCKED => 'נעולה',
        self::STATUS_CLOSED => 'סגורה',
        self::STATUS_CANCELLED => 'מבוטלת'
      );
  ...
  public function Delete()
  {
    ...
    {
      $this->m_nLastOperationStatus = parent::OPERATION_STATUS_VALIDATION_FAILED;
      $g_oError->AddError('לא ניתן למחוק הזמנה פעילה.');
      return FALSE;
    }
  ...
  public function Validate()
  {
    ...
    if (!$this->ValidateRequiredNames(self::PROPERTY_NAMES, 'כותרת הזמנה'))
      $bValid = FALSE;
    
    if (!$this->ValidateStatus())
    {
      $g_oError->AddError( sprintf('לא ניתן לשנות את %s לערך שנבחר', 'מצב'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_START] == NULL || get_class($this->m_aData[self::PROPERTY_START]) != "DateTime")
    {
      $g_oError->AddError( sprintf('%s חייב להכיל תאריך תקין', 'פתיחה'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_END] == NULL || get_class($this->m_aData[self::PROPERTY_END]) != "DateTime")
    {
      $g_oError->AddError( sprintf('%s חייב להכיל תאריך תקין', 'סגירה'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_DELIVERY] == NULL || get_class($this->m_aData[self::PROPERTY_DELIVERY]) != "DateTime")
    {
      $g_oError->AddError( sprintf('%s חייב להכיל תאריך תקין', 'משלוח'));
      $bValid = FALSE;
    }
    ...
    if ($bValid) //if dates are valid
    {
      ...
      {
        $g_oError->AddError( sprintf('%1$s צריך להיות מאוחר יותר או זהה ל-%2$s', 'סגירה',
                'פתיחה'));
        $bValid = FALSE;
      }
      ...
      if (intval($oInterval->format('%R%a')) < 0)
      {
        $g_oError->AddError( sprintf('%1$s צריך להיות מאוחר יותר או זהה ל-%2$s', 'משלוח',
                'סגירה'));
        $bValid = FALSE;
      }
    }
    ...
    {
      $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'עמלת קואופרטיב'));
      $bValid = FALSE;
    }
    ...
    if ($this->m_aData[self::PROPERTY_SMALL_ORDER] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'מכסת הזמנה קטנה'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_SMALL_ORDER_COOP_FEE] == NULL)
      {
        $g_oError->AddError( sprintf('יש להזין ערך עבור %1$s כאשר מוזן ערך עבור %2$s.',
                'עמלה להזמנה קטנה', 'מכסת הזמנה קטנה'
                ));
        $bValid = FALSE;
      }
    }
    ...
    if ($this->m_aData[self::PROPERTY_SMALL_ORDER_COOP_FEE] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'עמלה להזמנה קטנה'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_SMALL_ORDER] == NULL)
      {
        $g_oError->AddError( sprintf('יש להזין ערך עבור %1$s כאשר מוזן ערך עבור %2$s.', 'מכסת הזמנה קטנה',
                'עמלה להזמנה קטנה'
                ));
        $bValid = FALSE;
      }
    ...
    {
      $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'עמלה %'));
      $bValid = FALSE;
    }
    ...
    if ($this->m_aData[self::PROPERTY_MAX_COOP_TOTAL] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', ''));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_MAX_COOP_TOTAL] < 0)
      {
        $g_oError->AddError( sprintf('%s לא יכול להכיל ערך שלילי', ''));
        $bValid = FALSE;
      }
    }
