  ...
  public function __construct($nItems, $fItemQuantity, $sItemUnitAbbrev, $fUnitInterval, $sUnitAbbrev, $fPackageSize, $fQuantity, $fMaxCoopTotal, $fCoopTotal) {
    ...
    if ($fMaxCoopTotal > 0)
    {
      ...
      if ($this->m_aData[self::PROPERTY_AVAILABLE_ITEMS] > 0)
        $this->m_sToolTipAvailableItems = sprintf('%s: %s פריטים<br/>', 'נותרו', $this->m_aData[self::PROPERTY_AVAILABLE_ITEMS]);
      else
      {
        $this->m_aData[self::PROPERTY_SOLD_OUT] = TRUE;
        $this->m_sToolTipAvailableItems = 'המוצר אזל<br/>';
      }
    }
    ...
    else
    {
      ...
      {
        $this->m_sToolTipQuantityIntervalLine = '<div>הזמנה בהפרשים של‏:‏&nbsp;</div>' . 
      ...
      {                          
        ...
        $this->m_sToolTipPackageSizeLine = '<div>גודל חבילה‏:‏&nbsp;</div>' . 
