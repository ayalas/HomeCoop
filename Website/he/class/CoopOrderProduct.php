...
class CoopOrderProduct extends CoopOrderSubRecordBase {
  ...
  protected function Validate()
  {
    ...
    if ($this->m_aData[self::PROPERTY_PRODUCT_ID] == 0)
    {
      $g_oError->AddError( sprintf('יש להזין %s', 'מוצר'));
      $bValid = FALSE;
    }
    ...
    if ($this->m_aData[self::PROPERTY_PRODUCER_PRICE] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'מחיר יצרן'));
        $bValid = FALSE;
      }
    ...
    if ($this->m_aData[self::PROPERTY_COOP_PRICE] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'מחיר קואופרטיב'));
        $bValid = FALSE;
      }
    ...
    if ($this->m_aData[self::PROPERTY_MAX_USER_ORDER] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'מכסת הזמנה לחבר/ה'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_MAX_USER_ORDER] < 0)
      {
        $g_oError->AddError( sprintf('%s לא יכול להכיל ערך שלילי', 'מכסת הזמנה לחבר/ה'));
        $bValid = FALSE;
      }
    }
    ...
    if ($this->m_aData[self::PROPERTY_MAX_COOP_ORDER] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'מכסת הזמנה לקואופ'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_MAX_COOP_ORDER] < 0)
      {
        $g_oError->AddError( sprintf('%s לא יכול להכיל ערך שלילי', 'מכסת הזמנה לקואופ'));
        $bValid = FALSE;
      }
    }
    ...
    if ($this->m_aData[self::PROPERTY_BURDEN] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s חייב להכיל ערך מספרי', 'מעמסה'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_BURDEN] < 0)
      {
        $g_oError->AddError( sprintf('%s לא יכול להכיל ערך שלילי', 'מעמסה'));
        $bValid = FALSE;
      }
    }
    ...
    {
        $g_oError->AddError( 'כבר יש הזמנות עבור המוצר הזה בהזמנת הקואופרטיב הנוכחית. לא ניתן לשנות מחירים במצב כזה. יש להסיר תחילה את ההזמנות מהמוצר.');
        $bValid = FALSE;
    }
    ...
    if ($this->m_aOriginalData[self::PROPERTY_TOTAL_COOP_ORDER] > 0)
    {
      ...
      foreach($this->m_aOriginalData[self::PROPERTY_PICKUP_LOCATIONS_STORAGE] as $PLID => $Orig)
      {
        ...
        if (empty($nNewStorageAreaID))
        {
          ...
          {
            $g_oError->AddError(
                sprintf('יש מוצרים מוזמנים במקום האיסוף %s ולכן לא ניתן לבטל אותו.',
