...
class OrderItems extends SQLBase {
  ...
  {   
    ...
    $sSQL = " SELECT OI.OrderItemID, PRD.ProductKeyID, P.ProducerKeyID, PRD.UnitKeyID, " . 
        ...
        /*OID.mCoopPrice DeletedCoopTotal, OID.fMaxFixQuantityAddition DeletedQuantityAddition, OID.sMemberComments DeletedComments, " . */
          ...
          " LEFT JOIN T_OrderItem OI ON OI.OrderID = O.OrderID AND OI.ProductKeyID = COPRD.ProductKeyID " .
          " LEFT JOIN T_OrderItem_Deleted OID ON OID.OrderID = O.OrderID AND OID.ProductKeyID = COPRD.ProductKeyID AND OI.OrderItemID IS NULL " .
 ...
 protected function ValidateData()
 {
   ...
   foreach($this->m_aData[self::PROPERTY_ORDER_ITEMS] as $nProductID => $oOrderItem)
   {
     ...
     if ($g_oMemberSession->MemberID != $this->m_oOrder->MemberID)
     {
       if (!$this->SetRecordGroupID(self::PERMISSION_MODIFY_ORDER_ITEMS, $oOriginalItem->CoordinatingGroupID, FALSE))
       {
         ...
         $oOrderItem->ValidationMessage .= 'הגישה נחסמה<br/>'; 
       }
     }
     ...
     {  
       ...
       {
         $oOrderItem->InvalidEntry = TRUE;
         $oOrderItem->ValidationMessage .= 'לא ניתן להסיר כרגע הזמנה ממוצר זה, כיוון שנעשה עבורו איחוד פריטים לחבילה גדולה יותר. אנא פנו למתאמי ההזמנה/קואופרטיב<br/>'; 
       }
       if ($oOrderItem->MemberMaxFixQuantityAddition != NULL)
       {
          $oOrderItem->InvalidEntry = TRUE;
          $oOrderItem->ValidationMessage .= 'לא ניתן להזין ערך לעמודה הוספה ללא הזמנה<br/>';  
       }
   ...
   foreach($this->m_aStorageAreasBurden as $StorageID => $aStorage)
   {
    ...
    {
      $bValid = FALSE;
      $g_oError->AddError(sprintf('לא ניתן לשמור את ההזמנה המבוקשת כי אין מספיק מקום אחסון פנוי ב%s', $aStorage['StorageAreaName']));
 ...
 protected function InsertItem(&$oOrderItem)
 {
   ...
   //delete deleted item if existed
   if ($oOrderItem->DeletedItemID > 0)
   {
 ...
 protected function DeleteItem(&$oOrderItem)
 {
   ...
   //backup deleted item if not done by order member
   if ($this->m_oOrder->MemberID != $g_oMemberSession->MemberID)
   {
    $sSQL = "INSERT INTO T_OrderItem_Deleted (OrderItemID, OrderID, ProductKeyID, fQuantity, mCoopPrice, mProducerPrice, fOriginalQuantity, ". 
            ...
            " FROM T_OrderItem OI " .
            " WHERE OI.OrderItemID = " . $oOrderItem->OrderItemID .
            " ON DUPLICATE KEY UPDATE OrderID = " . $this->m_oOrder->ID . ", ProductKeyID = " . $oOrderItem->ProductID . ";";
    $this->RunSQL($sSQL);
 ...
 protected function ValidateProducer(&$recProducer)
 {
     ...
     if ($mChangedTotal > $recProducer["mProducerTotal"])
     {
        ...
        if ($mChangedTotal > $recProducer["mMaxProducerOrder"])
        {
          ...
          if ($this->m_oOrder->HasPermission(SQLBase::PERMISSION_COORD))
            $g_oError->AddError(sprintf('שדה סכום ליצרן עבור היצרן %s של הזמנת הקואופרטיב חרג מהמגבלה שהוגדרה בשדה מכסת הזמנה ליצרן.',$recProducer["sProducer"]));
          $bValid = FALSE;
        }
     }
   }
   ...
   if (!$bValid)
     $g_oError->AddError(sprintf('לא ניתן להגדיל את ההזמנה הזו מעבר לתפוסה המלאה של היצרן %s של הזמנת הקואופרטיב',$recProducer["sProducer"]));
 ...
 protected function ValidateProduct(&$oOrderItem, &$oOriginalItem)
 {
   ...
   {
     $oOrderItem->InvalidEntry = TRUE;
     $oOrderItem->ValidationMessage .= 'לא ניתן להפחית כמות כרגע ממוצר זה, כיוון שנעשה עבורו איחוד פריטים לחבילה גדולה יותר לצרכי חסכון עלויות. אנא פנו למתאמי ההזמנה/קואופרטיב<br/>';
   }
   ...
   if ($oOrderItem->ProductMaxUserOrder != NULL)
   {
     ...
     {
        $oOrderItem->InvalidEntry = TRUE;
        $oOrderItem->ValidationMessage .= 'הכמות שהוזנה חורגת ממכסת ההזמנה פר חבר/ה עבור המוצר<br/>';
     }
   }
