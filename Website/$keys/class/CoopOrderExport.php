  ...
  public function GetDataSetsList()
  {
    global $g_oMemberSession;
             ...
             FALSE))
     {
        ...
        $aReturn[( self::LIST_ITEM_TYPE_ORDER ) ] = '<!$ORDER_EXPORT_ENTIRE_COOP_ORDER$!>';
        $aReturn[( self::LIST_ITEM_TYPE_ORDER + self::LIST_ITEM_TYPE_SUMMARY ) ] = '<!$ORDER_EXPORT_COOP_ORDER_SUMMARY$!>';
     
        if ($bHasOrdersPermission)
          $aReturn[ ( self::LIST_ITEM_TYPE_MAILS ) ] = '<!$ORDER_EXPORT_ENTIRE_COOP_ORDER_MAIL_LIST$!>';
     }
     ...
     if ($bHasOrdersPermission)
      $aReturn[ (self::LIST_ITEM_PRODUCTS + self::LIST_ITEM_TYPE_MAILS)  ] = '<!$ORDER_EXPORT_ENTIRE_COOP_ORDER_PRODUCTS_MAIL_LIST$!>';
 ...
 {   
  ...
  public function EchoXML()
  {
    ...
    else if ($this->m_aData[self::PROPERTY_ID] == CoopOrderExport::LIST_ITEM_TYPE_ORDER)
    {
     ...
     $sFileName .= '<!$ORDER_EXPORT_COOP_ORDER_ALL_FILE_NAME_SUFFIX$!>';
    }
    else
      return;
  ...
  protected function BuildNewSheetXML($sSheetName, $bIncludeOrders, &$sheet, &$batch)
  {
    ...
    $totalh = $this->m_oXmlDoc->createElement('totalh', '<!$FIELD_ORDER_EXPORT_TOTAL_PRODUCT_QUANTITY$!>');
    $colh->appendChild($totalh);
    
    if ($this->m_nProducerID > 0)
    {
      $totalph = $this->m_oXmlDoc->createElement('totalh', '<!$FIELD_ORDER_EXPORT_TOTAL_PRODUCT_PRODUCER_SUM$!>');
  ...
  protected function BuildPickupLocationDetailSheetXML(&$sheet)
  {
    ...
    foreach($this->m_aOrderItems as $OrderID => $products)
    {
      ...
      //summary row
      $sum = $this->m_oXmlDoc->createElement('sum');
      $sumlabel = $this->m_oXmlDoc->createElement('sumlabel', sprintf('<!$LABEL_EXPORT_TOTAL_MEMBER$!>', $order["sName"]));
      $sum->appendChild($sumlabel);
