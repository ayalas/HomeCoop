...
class CoopOrderProduct extends CoopOrderSubRecordBase {
  ...
  protected function Validate()
  {
    ...
    if ($this->m_aData[self::PROPERTY_PRODUCT_ID] == 0)
    {
      $g_oError->AddError( sprintf('<!$FIELD_REQUIRED$!>', '<!$FIELD_PRODUCT$!>'));
      $bValid = FALSE;
    }
    ...
    if ($this->m_aData[self::PROPERTY_PRODUCER_PRICE] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('<!$FIELD_MUST_BE_NUMERIC$!>', '<!$FIELD_PRODUCER_PRICE$!>'));
        $bValid = FALSE;
      }
    ...
    if ($this->m_aData[self::PROPERTY_COOP_PRICE] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('<!$FIELD_MUST_BE_NUMERIC$!>', '<!$FIELD_COOP_PRICE$!>'));
        $bValid = FALSE;
      }
    ...
    if ($this->m_aData[self::PROPERTY_MAX_USER_ORDER] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('<!$FIELD_MUST_BE_NUMERIC$!>', '<!$FIELD_MAX_ORDER_PER_MEMBER$!>'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_MAX_USER_ORDER] < 0)
      {
        $g_oError->AddError( sprintf('<!$FIELD_NON_NEGATIVE$!>', '<!$FIELD_MAX_ORDER_PER_MEMBER$!>'));
        $bValid = FALSE;
      }
    }
    ...
    if ($this->m_aData[self::PROPERTY_MAX_COOP_ORDER] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('<!$FIELD_MUST_BE_NUMERIC$!>', '<!$FIELD_MAX_ORDER_PER_COOP$!>'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_MAX_COOP_ORDER] < 0)
      {
        $g_oError->AddError( sprintf('<!$FIELD_NON_NEGATIVE$!>', '<!$FIELD_MAX_ORDER_PER_COOP$!>'));
        $bValid = FALSE;
      }
    }
    ...
    if ($this->m_aData[self::PROPERTY_BURDEN] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('<!$FIELD_MUST_BE_NUMERIC$!>', '<!$FIELD_BURDEN$!>'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_BURDEN] < 0)
      {
        $g_oError->AddError( sprintf('<!$FIELD_NON_NEGATIVE$!>', '<!$FIELD_BURDEN$!>'));
        $bValid = FALSE;
      }
    }
      ...
      foreach($this->m_aOriginalData[self::PROPERTY_PICKUP_LOCATIONS_STORAGE] as $PLID => $Orig)
      {
        ...
        else if ($Orig['Data']['StorageAreaKeyID'] != $nNewStorageAreaID && $this->m_aData[self::PROPERTY_BURDEN] > 0) 
        {
          ...
          if ($rec != NULL && $rec['fTotalQuantity'] != NULL)
          {
            ...
            {
              $g_oError->AddError(
                  sprintf('<!$PRODUCT_VALIDATION_BURDEN_EXCEEDS_STORAGE_BURDEN$!>',
                      htmlspecialchars($Orig['Data']['sPickupLocation'])));
              $bValid = FALSE;
            }
          }
