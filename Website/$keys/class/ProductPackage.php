  ...
  public function __construct($nItems, $fItemQuantity, $sItemUnitAbbrev, $fUnitInterval, $sUnitAbbrev, $fPackageSize, $fQuantity, $fMaxCoopTotal, $fCoopTotal,
    ...
    if ($fMaxCoopTotal > 0)
    {
      ...
      if ($this->m_aData[self::PROPERTY_AVAILABLE_ITEMS] > 0)
        $this->m_sToolTipAvailableItems = sprintf('<div><!$TOOLTIP_ITEMS$!></div>', '<!$ITEMS_LEFT$!>', $this->m_aData[self::PROPERTY_AVAILABLE_ITEMS]);
      else
      {
        $this->m_aData[self::PROPERTY_SOLD_OUT] = TRUE;
        $this->m_sToolTipAvailableItems = '<div><!$TOOLTIP_SOLD_OUT$!></div>';
      }
    }
    ...
    else
    {
      ...
      {
        $this->m_sToolTipQuantityIntervalLine = '<div><!$FIELD_UNIT_INTERVAL$!><!$FIELD_DISPLAY_NAME_SUFFIX$!>&nbsp;<!$LANGUAGE_DIRECTION_MARK$!>' . 
      ...
      {                          
        ...
        $this->m_sToolTipPackageSizeLine = '<div><!$FIELD_PACKAGE_SIZE$!><!$FIELD_DISPLAY_NAME_SUFFIX$!>&nbsp;<!$LANGUAGE_DIRECTION_MARK$!>' . 
