...
class CoopOrderProducer extends CoopOrderSubRecordBase {
  ...
  public function Delete()
  {
    ...
    {
      $this->m_nLastOperationStatus = parent::OPERATION_STATUS_VALIDATION_FAILED;
      $g_oError->AddError('<!$COOP_ORDER_PRODUCER_VALIDATION_CANNOT_DELETE_WHEN_HAS_ORDERS$!>');
      return FALSE;
    }
  ...
  public function Validate()
  {
    ...
    if ($this->m_aData[self::PROPERTY_MAX_PRODUCER_ORDER] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('<!$FIELD_MUST_BE_NUMERIC$!>', '<!$FIELD_MAX_PRODUCER_ORDER$!>'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_MAX_PRODUCER_ORDER] < 0)
      {
        $g_oError->AddError( sprintf('<!$FIELD_NON_NEGATIVE$!>', '<!$FIELD_MAX_PRODUCER_ORDER$!>'));
        $bValid = FALSE;
      }
    }
