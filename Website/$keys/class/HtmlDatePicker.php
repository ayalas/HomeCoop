...
class HtmlDatePicker {
  ...
  public function EchoHtml()
  {
    ...
    echo ' for="' , $sDateId , '">' , $this->m_aData[self::PROPERTY_LABEL] , '<!$FIELD_DISPLAY_NAME_SUFFIX$!></label></td>';

    echo '<td nowrap><input type="text" maxlength="<!$MAX_LENGTH_DATE_PICKER_DATE_PART$!>" id ="' , $sDateId , '" name = "' , $sDateId , 
            '" value="';
    if ($this->m_aData[self::PROPERTY_VALUE] != NULL)
    {
      echo $this->m_aData[self::PROPERTY_VALUE]->format('<!$DATE_PICKER_DATE_FORMAT$!>');
    }
    ...
    if ($this->m_aData[self::PROPERTY_TIME_SETTING] != self::TIME_NOT_DISPLAYED)
    {
      ...
      echo '<input type="text" maxlength="<!$MAX_LENGTH_DATE_PICKER_TIME_PART$!>" id="' , $sTimeId , '" name="' , $sTimeId , '" value="';
      if ($this->m_aData[self::PROPERTY_VALUE] != NULL)
         echo $this->m_aData[self::PROPERTY_VALUE]->format('<!$DATE_PICKER_TIME_FORMAT$!>');
      else if ($_SERVER[ 'REQUEST_METHOD'] == 'POST')
      {
