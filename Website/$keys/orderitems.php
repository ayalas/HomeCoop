...
function SwitchViewMode()
{
  ...
  if (document.getElementById("hidDirty").value == 1)
    bConfirm = confirm(decodeXml('<!$ORDER_ITEMS_NOT_SAVED_BEFORE_VIEW_CHANGE$!>'));
...
<table cellspacing="0" cellpadding="0" width="100%">
    ...
    <tr >
                ...
                {
                ?><tr>
                  <td class="ordercnt" class="nowrapping"><div class="inlineblock"><button type="submit" class="order" onclick="JavaScript:Save();" id="btn_save" name="btn_save" ><!$BTN_SAVE_ORDER_ITEMS$!></button></div>
                    ...
                    <?php
                      ...
                      if ($oTable->ProductsViewMode == OrderItems::PRODUCTS_VIEW_MODE_SHOW_ALL)
                        echo ' selected ';
                      echo '><!$ORDER_ITEMS_VIEW_MODE_ALL_PRODUCTS$!></option>',
                       '<option value="' , OrderItems::PRODUCTS_VIEW_MODE_ITEMS_ONLY , '"';
                      if ($oTable->ProductsViewMode == OrderItems::PRODUCTS_VIEW_MODE_ITEMS_ONLY)
                        echo ' selected ';
                      echo '><!$ORDER_ITEMS_VIEW_MODE_ORDERED_PRODUCTS$!></option>';
                    ?>
                    </select></div>
                  </td>
                </tr>
...
<?php
                ...
                {
                  ?>
                  <div class="norecords"><!$NO_RECORD_FOUND$!></div>
                  <?php
                }
                else
                {
                  ...
                  foreach($arrItems as $oItem)
                  {
                      ...
                      if ($oItem->DisabledProduct)
                      {
                        echo '<div class="resgriddatalong"><!$CANCELLED_PRODUCT$!></div>';
                      }
                      else
                      {
                        echo '<div class="resgriddatalong">';
