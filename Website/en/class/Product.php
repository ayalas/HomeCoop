...
class Product extends SQLBase {
  ...
  {   
    ...
    {
      $sMessage = sprintf('Upload file %s failed.', $oUploader->OriginalFileName);
      //analize errors
      switch($nResponse)
      {
        case FileUploader::RESPONSE_FILE_OBJECT_NOT_SET:
          $sMessage .= ' It seems that the reason is a bug in the software';
          break;
        case FileUploader::RESPONSE_MAX_FILE_SIZE_EXCEEDED:
          $sMessage .= sprintf(' The maximum size of %dKb was exceeded', (PRODUCT_IMAGE_MAX_FILE_SIZE/1024)); //1024 bytes = 1 KB
          break;
        case FileUploader::RESPONSE_FILE_TYPE_UNSUPPORTED:
          $sMessage .= ' The file type or size is not supported.';
          break;
        case FileUploader::RESPONSE_CANT_WRITE:
          $sMessage .= ' There seems to be a permission error when uploading to this site.';
          break;
        case FileUploader::RESPONSE_NO_TMP_DIR:
          $sMessage .= ' There seems to be a permission or configuration error when uploading to this site: a temporary destination folder is not found.';
          break;
        case FileUploader::RESPONSE_UPLOADS_DIR_WRITE_FAILED:
          $sMessage .= ' The file was uploaded, but its move to the site&#x27;s uploads folder failed, probably due to a site permission error.';
          break;
        case FileUploader::RESPONSE_FILE_OBJECT_ERROR:
          $sMessage .= sprintf(' Error code(s): %s', implode(',', $oFile['error']));
          break;
      }
  ...
  public function Validate()
  {
    ...
    if (!$this->ValidateRequiredNames(self::PROPERTY_PRODUCT_NAMES, 'Product Name'))
      $bValid = FALSE;
   ...
   /* if (!$this->ValidateRequiredNames(self::PROPERTY_SPEC_STRINGS, 'Details'))
      $bValid = FALSE;*/
    
    if ($this->m_aData[self::PROPERTY_UNIT_ID] <= 0)
    {
      $g_oError->AddError(sprintf('Must select %s', 'Unit'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_QUANTITY] === NULL)
    {
      $g_oError->AddError(sprintf('%s is required.', 'Quantity'));
      $bValid = FALSE;
    }
    else if ($this->m_aData[self::PROPERTY_QUANTITY] <= 0)
    {
      $g_oError->AddError(sprintf('%s cannot have a negative value or zero', 'Quantity'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_PACKAGE_SIZE] != NULL && $this->m_aData[self::PROPERTY_PACKAGE_SIZE] <= 0)
    {
      $g_oError->AddError(sprintf('%s cannot have a negative value or zero', 'Package Size'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_ITEMS_IN_PACKAGE] === NULL)
    {
      $g_oError->AddError(sprintf('%s is required.', 'Items in Package'));
      $bValid = FALSE;
    }
    else if ($this->m_aData[self::PROPERTY_ITEMS_IN_PACKAGE] <= 0)
    {
      $g_oError->AddError(sprintf('%s cannot have a negative value or zero', 'Items in Package'));
      $bValid = FALSE;
    }
    ...
    {
      $g_oError->AddError(sprintf('%s cannot have a negative value or zero', 'Item Quantity'));
      $bValid = FALSE;
    }     
    ...
    if ($this->m_aData[self::PROPERTY_ITEM_QUANTITY] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s must be numeric.', 'Item Quantity'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_ITEM_UNIT_ID] == NULL)
      {
        $g_oError->AddError( sprintf('%1$s must be set when %2$s is set.', 'Item Unit',
                'Item Quantity'
                ));
        $bValid = FALSE;
      }
    ...
    {
      $g_oError->AddError(sprintf('%s cannot have a negative value or zero', 'Unit Interval'));
      $bValid = FALSE;
    }
    ...
    {
      $g_oError->AddError(sprintf('%s cannot have a negative value', 'Max. Order'));
      $bValid = FALSE;
    }
    
    if ($this->m_aData[self::PROPERTY_PRODUCER_PRICE] === NULL)
    {
      $g_oError->AddError(sprintf('%s is required.', 'Producer Price'));
      $bValid = FALSE;
    }
    ...
    {
      $g_oError->AddError(sprintf('%s is required.', 'Coop. Price'));
      $bValid = FALSE;
    }
    ...
    {
      $g_oError->AddError(sprintf('%s is required.', 'Burden'));
      $bValid = FALSE;
    }
    ...
    {
      $g_oError->AddError('Product Unit must be set to Items when a Join To Product is defied');
      $bValid = FALSE;
    }
