...
class CoopOrderPickupLocation extends CoopOrderSubRecordBase {
  ...
  public function Validate($bNewRecord)
  {
    ...
    if ($this->m_aData[self::PROPERTY_MAX_COOP_TOTAL] != NULL)
    {
      ...
      {
        $g_oError->AddError( sprintf('%s must be numeric.', 'Max. Coop Total'));
        $bValid = FALSE;
      }
      else if ($this->m_aData[self::PROPERTY_MAX_COOP_TOTAL] < 0)
      {
        $g_oError->AddError( sprintf('%s cannot have a negative value', 'Max. Coop Total'));
        $bValid = FALSE;
      }
    }
  ...
  protected function ValidateStorageArea($nStorageAreaKeyID, $aStorageArea, $bNewRecord)
  {
    ...
    if (!$bNewRecord)
    {
      ...
      if ($aStorageArea['bDisabled'] && !$this->m_aOriginalData[self::PROPERTY_STORAGE_AREAS][$nStorageAreaKeyID]['bDisabled'])
      {
        ...
        if ($sOrderedProducts != '')
          $g_oError->AddError(sprintf('%s is associated with the following ordered products and cannot be deactivated: %s',
                $aStorageArea['sStorageArea'], $sOrderedProducts));
        if ($sNonOrderedProducts != '')
          $g_oError->AddError(sprintf('%s is associated with the following products - these products will not appear in the order page unless associated with an active storage area: %s',
