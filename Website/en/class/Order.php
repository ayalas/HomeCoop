  ...
  protected function CanModify()
  {
    ...
    if (!$this->m_bRunningTotalsValidation)
    {
      ...
      if ($this->m_aData[self::PROPERTY_COOP_ORDER_STATUS_OBJECT]->Status != ActiveCoopOrderStatus::Open)
      {
        if (!$this->HasPermission(self::PERMISSION_COORD))
        {
          $this->AddError('This order cannot be modified because its cooperative order is in full capacity.');
          return FALSE;
        }
        else
          $this->AddError('Member permission was bypassed: This order cannot be modified because its cooperative order is in full capacity.'); 
      }
    ...
    if ($mMaxOrder != NULL) //if not payment at pickup
    {
      if ($mMaxOrder < $this->m_aData[self::PROPERTY_COOP_TOTAL])
      {
        ...
        if (!$this->HasPermission(self::PERMISSION_COORD))
        {
          $this->AddError(sprintf('Order cannot be enlarged beyond a total products of %s',$mMaxOrder));
          return TRUE; //CanEnlarge remains FALSE
        }
        else
          $this->AddError(sprintf('Member permission was bypassed: Order cannot be enlarged beyond a total products of %s',$mMaxOrder)); 
      }
    }
