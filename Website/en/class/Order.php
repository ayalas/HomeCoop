...
class Order extends SQLBase {
 ...
 public function ReductFromBalance()
 {
  ...
  $this->RunSQLWithParams($sSQL, array(
            ...
            'date' => $sNow,
            'desc' => 'auto-payment from balance',
          ));
 ...
 protected function ReductFromBalanceOnPLChange()
 {
  ...
  $this->RunSQLWithParams($sSQL, array(
            ...
            'date' => $sNow,
            'desc' => 'Pickup Location Change Source',
          ));
  ...
  $this->RunSQLWithParams($sSQL, array(
            ...
            'amount' => (-$this->m_aData[self::PROPERTY_COOP_TOTAL_INC_FEE]),
            'date' => $sNow,
            'desc' => 'Pickup Location Change Target',
          ));
  ...
  protected function CanModify()
  {
    ...
    if (!$this->m_bRunningTotalsValidation)
    {
      ...
      if ($this->m_aData[CoopOrder::PROPERTY_STATUS] != CoopOrder::STATUS_ACTIVE)
      {
        if ($bHasCoordPermission && 
          ...
          $this->AddError('Member permission was bypassed: This order cannot be modified because its cooperative order is not active.', 'warning'); 
        else
        {
          $this->AddError('This order cannot be modified because its cooperative order is not active.');
          return FALSE;
        }
      }
      ...
      if ($this->m_aData[self::PROPERTY_COOP_ORDER_STATUS_OBJECT]->Status != ActiveCoopOrderStatus::Open)
      {
        if (!$bHasCoordPermission)
        {
          $this->AddError('This order cannot be modified due to its date range limitations.');
          return FALSE;
        }
        else
          $this->AddError('Member permission was bypassed: This order cannot be modified due to its date range limitations.', 'warning'); 
      }
    ...
    {    
      if ($this->IsLarger())
      {
        ...
        if ($this->m_aData[self::PROPERTY_ID] > 0)
          $this->AddError('This order cannot be enlarged beyond the full capacity of the cooperative order.');

        //more detailed message for coordinators
        if ($bHasCoordPermission)
        {
          if ($oCoopOrderCapacity->SelectedType == CoopOrderCapacity::TypeBurden)
            $this->AddError('The reason for the full capacity is that the field Total Burden has reached or exeeded the value of Delivery Capacity in the cooperative order.');
          else if ($oCoopOrderCapacity->SelectedType == CoopOrderCapacity::TypeTotal)
            $this->AddError('The reason for the full capacity is that the field Total Coop has reached or exeeded the value of Max. Coop Total‏ in the cooperative order.');
        }
      }

      if ($this->m_aData[self::PROPERTY_ID] == 0)
      {
        $this->AddError('Cannot create a new order because the cooperative order is in full capacity.');
    ...
    if ($this->m_aData[self::PROPERTY_MAX_ORDER] != NULL) //if not payment at pickup
    {
      ...
      if ($mSumForValidation < 0)
      {
        ...
        {
          $this->AddError(sprintf('Order cannot be enlarged beyond a total products of %s',$this->m_aData[self::PROPERTY_MAX_ORDER]));
          return TRUE; //CanEnlarge remains FALSE
        }
        else
          $this->AddError(sprintf('Member permission was bypassed: Order cannot be enlarged beyond a total products of %s',
  ...
  public function BuildPageTitle()
  {
    ...
    if ($this->m_aData[self::PROPERTY_ID] > 0)
    {
      ...
      {
        $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION] = 'My Order';
      ...
      {
        $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION] = sprintf('%s&#x27;s Order', $this->m_aData[self::PROPERTY_MEMBER_NAME]);
        $this->m_aData[self::PROPERTY_PAGE_TITLE] = $this->m_aData[self::PROPERTY_COOP_ORDER_NAME] . ' - ' . 
                $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION];
      }
    }
    ...
    {
      $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION] = 'New Order';
      $this->m_aData[self::PROPERTY_PAGE_TITLE] = $this->m_aData[self::PROPERTY_COOP_ORDER_NAME] . ' - ' . 
                $this->m_aData[self::PROPERTY_PAGE_TITLE_ADDITION];
    }
  }
