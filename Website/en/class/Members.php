...
class Members extends SQLBase {
 ...
 {
   return array( self::EXPORT_LIST_ITEM_ALL_MEMBERS_DATA => 'All members data',
       self::EXPORT_LIST_ITEM_SELECTED_MEMBERS_EMAILS => 'Emails of selected members'
    );
 }
 ...
 {    
  ...
  public function EchoXML()
  {
    ...
    $sFileName = $g_dNow->format('Y_m_d') . '_Members.ods';
  ...
  protected function BuildXmlDoc()
  {
    ...
    $sheetname = $this->m_oXmlDoc->createElement('name', 'Members');
    $sheet->appendChild($sheetname);
    
    $colh = $this->m_oXmlDoc->createElement('colh');
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'Name');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'User name');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'Balance');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'Balance Held');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'Balance Invested');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'Payment Method');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', '% Over Balance');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'Email address');
    $colh->appendChild($ch);
    
    $ch = $this->m_oXmlDoc->createElement('colheader', 'Joined On');
    $colh->appendChild($ch);
    ...
    while($recMember)
    {
      ...
      $rd = $this->m_oXmlDoc->createElement('djoin', $dJoined->format('n.j.Y') );
      $row->appendChild($rd);
