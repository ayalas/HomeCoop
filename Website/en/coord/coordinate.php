...
function ProcessMember(&$recMember, $bMember)
{
  ...
  {
    echo '<span class="bolddata">Coordinated Solely by this Member</span>&nbsp;';
    ...
    if ($oCoordinate->GetPermissionScope(SQLBase::PERMISSION_COORD) == Consts::PERMISSION_SCOPE_COOP_CODE  && !$g_oError->HadError)
      echo '<button type="button" id="btnRemoveGroup' , $recMember["MemberID"] , '" name="btnRemoveGroup' , $recMember["MemberID"] , 
          '" onclick="JavaScript:RemoveGroup();" >Remove Coordination</button>&nbsp;';
  }
    ...
    echo '<button type="button" id="btnAssignToMember' ,  $recMember["MemberID"] , '" name="btnAssignToMember' 
          ,  $recMember["MemberID"] , '" onclick="JavaScript:SetMemberAsCoordinator(' ,  $recMember["MemberID"] ,
            ');" >Set as Sole Coordinator</button>';
    ...
    if (!empty( $_POST['hidPostAction'] ))
    {
      ...
      switch($oCoordinate->LastOperationStatus)
      {
        ...
        case SQLBase::OPERATION_STATUS_REQUIRED_FIELD_MISSING:
          $g_oError->AddError(' Group name is required.');
          break;
        case SQLBase::OPERATION_STATUS_NO_LIST_ITEM_SELECTED:
          $g_oError->AddError(' No members were selected.');
          break;
        case SQLBase::OPERATION_STATUS_CANT_REMOVE_OWN_PERMISSION:
          $g_oError->AddError(' Cannot remove own coordination permissions, only share it with others.');
          break;
      }
    ...
    <tr>
        <td>
                ...
                <tr>
                  ...
                  <td width="100%">
                    <?php if ($recGroupList) { 
                      echo '<select id="selGroup" title="Select another Group" name="selGroup" ';
                      if ($bExtendedGroupPermissions) 
                        echo ' disabled="true" ';
                      echo ' onchange="JavaScript:LoadGroup();" ><option value="0">Select from list</option>';
                ...
                <table cellspacing="0" cellpadding="2" width="100%">
                <tr>
                  <td class="columntitle">Coordinator</td>
                  <td class="columntitle">In Group?</td>
                  <td class="columntitle">Is Contact Person?</td>
                ...
                {
                  echo '<tr><td colspan="4">&nbsp;</td></tr><tr><td colspan="4">To add more coordinators, please assign the relevant members coordinating permissions for the permission area</td></tr>';
                }
?>
                </table>
                </td>
